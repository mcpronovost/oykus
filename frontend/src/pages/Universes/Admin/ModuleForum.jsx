...
export default function OykUniverseAdminModuleForum() {
  ...
  const forumFormRef = useRef(null);
  ...
  const postSubmit = async () => {
    ...
    try {
      ...
      let settings = {};
      for (const [key, value] of Object.entries(forumForm)) {
        settings[key] = value.trim();
      }
      formData.append("settings", JSON.stringify(settings));
      const r = await api.post(`/world/universes/${currentUniverse.slug}/modules/forum/edit/`, formData);
      if (!r?.ok || !r.module) throw r;
  ...
  const handleReset = async () => {
    setHasError(null);
    setHasSuccessSubmit(null);
    forumFormRef.current?.reset();
  };

  useEffect(() => {
    const controller = new AbortController();

    routeTitle(`${t("Admin")} - ${t("Universe Forum Module")}`);
  ...
  return (
    <section className="oyk-universes-admin-profile">
      <OykHeading subtitle tag="h2" title={t("Forum Module")} nop />
      <OykCard>
        {isLoading ? (
          <OykLoading />
        ) : (
          <OykForm ref={forumFormRef} className="oyk-universes-admin-form" isLoading={isLoading} onSubmit={postSubmit}>
            <section>
              <OykFormField
                label={t("Display Name")}
                name="display_name"
                defaultValue={forumForm.display_name || t("Forum")}
