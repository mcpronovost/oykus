...
export default function ModalPostDelete({ postId, isOpen, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    setHasError(null);
    try {
      const r = await api.post(`/blog/u/${currentUniverse.slug}/posts/${postId}/delete/`);
      if (!r.ok) throw new Error(r.error || t("An error occurred"));
      onClose(true);
    } catch (e) {
      setHasError(e.message || t("An error occurred while deleting the post"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <OykModal title={t("Delete Post")} isOpen={isOpen} onClose={onClose}>
