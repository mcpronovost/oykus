...
import OykModalFriendsDelete from "./modals/FriendsDelete";

export default function SettingsFriendsCard({ friend, fetchData = () => {} }) {
  const { n } = useRouter();

  const [isModalFriendsDeleteOpen, setIsModalFriendsDeleteOpen] = useState(false);

  const handleModalFriendsDeleteClose = (updated) => {
    setIsModalFriendsDeleteOpen(false);
    if (updated) {
      fetchData();
    }
  };
  ...
  return (
    <li key={friend.slug} className="oyk-settings-friends-list-item">
      <OykModalFriendsDelete
        isOpen={isModalFriendsDeleteOpen}
